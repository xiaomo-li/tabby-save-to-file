...
export class SaveOutputDecorator extends TerminalDecorator {
  ...
  attach(tab: BaseTerminalTabComponent<any>): void {
    if (
      this.config.store.saveOutput.autoSave === "off" ||
      (this.config.store.saveOutput.autoSave === "ssh" &&
  ...
  private generatePath(tab: BaseTerminalTabComponent<any>): string {
    let outputPath =
      this.config.store.saveOutput.autoSaveDirectory || os.homedir();
    let outputName =
      new Date().toISOString() +
      " - " +
      (tab.customTitle || tab.title || "Untitled") +
      ".txt";
